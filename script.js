...
// CORRECTION CRITIQUE : L'API tourne sur le port 8001 

const API_BASE_URL = 'http://127.0.0.1:8001';  

let categoriesMap = {};  

let charts = {}; // Objet pour stocker les instances de Chart.js 
...
async function fetchSummaryAndAlerts(month, startDate, endDate) { 
    ...
    try { 
        ...
        // Appels parallèles pour les analyses 
        ...
        // Vérification du statut de toutes les réponses 
        ...
        // Mettre à jour la Map des catégories (pour afficher le nom dans le tableau) 
        ...
        // Affichage des Alertes (Rouge si alerts.length > 0) 
        ...
        // Mettre à jour la liste complète des transactions 
...
async function fetchAndDisplayTransactions() { 
    ...
    try { 
        ...
        transactions.forEach(tx => { 
            ...
            // CORRECTION: AJOUT DE LA CLASSE CSS POUR LES REVENUS 

            const amountCell = row.insertCell(); 

            amountCell.textContent = `${tx.amount.toFixed(2)} €`;  

            if (tx.amount < 0) { // Si le montant est négatif, c'est un revenu (vert) 
    ...
    if (confirm(`Êtes-vous sûr de vouloir supprimer la transaction ${id} ?`)) { 
        ...
        try { 
            ...
            if (response.status === 204) { 
                ...
                initDashboard(); // Recharger toutes les données 
...
async function handleImport() { 
    ...
    try { 
        ...
        if (response.ok) { 
            ...
            initDashboard(); // Recharger toutes les données après import 
...
function drawCategoryChart(categories) { 
    ...
    // Filtrer les revenus pour le graphique de répartition des DÉPENSES 
...
function initDashboard() { 
    ...
    // CORRECTION CRITIQUE : Appeler fetchSummaryAndAlerts au lieu de fetchAndDisplayDashboard 
...
// Attend que le HTML soit chargé avant d'exécuter initDashboard() 
